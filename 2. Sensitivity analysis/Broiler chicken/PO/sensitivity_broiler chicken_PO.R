## Scenario:Broiler chickens following oral gavage 50mg/kg for 10 doses wihthin 5 days
...
setwd("C:/Users/zhichengzhang/Desktop/Tyloasin_08_18/Figures/Figure 7_Sensitivity analysis/Broiler chicken/PO")
...
OUT<- pred(pars, tinterval = 12, TDOSE = 10, BW = 2.2, route = 'oral gavage', Dose =50)
...
NSC_func <- function (Gpars, pred) {
  ...
  for (i in 1:nG) {
    ...
    Rnew.G         <- pred(Gpars.new, tinterval = 12, TDOSE = 10, BW = 2.2, route = 'oral gavage', Dose =50)
    R.G            <- pred(Gpars, tinterval = 12, TDOSE = 10, BW = 2.2, route = 'oral gavage', Dose =50)
...
heatmap1 <- ggplot(ttt_long, aes(x = variable, y = X, fill = value)) + 
  geom_tile() + 
  ...
  labs(title = "(A) Normalized Sensitivity Coefficients for broiler chicken (PO)", x = "Target tissues", y = "Parameters") +
  theme_minimal() +
