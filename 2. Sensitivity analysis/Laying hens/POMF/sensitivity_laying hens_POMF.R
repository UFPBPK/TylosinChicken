## Scenario:laying hens following POMF administration 50mg/kg for 10 consecutive days wihthin 5 days
...
setwd("C:/Users/zhichengzhang/Desktop/Tyloasin_08_18/Figures/Figure 7_Sensitivity analysis/Laying hens/POMF")
...
OUT<- pred(log(pars), TDOSE = 10, BW = 2.2, route = 'oral gavage', Dose =50, tlay1 = 12)
...
NSC_func <- function (Gpars, pred) {
  ...
  for (i in 1:nG) {
    ...
    Rnew.G         <- pred(Gpars.new,  TDOSE = 10, BW = 2.2, route = 'oral gavage', Dose =50, tlay1 = 12)
    R.G            <- pred(Gpars,  TDOSE = 10, BW = 2.2, route = 'oral gavage', Dose =50, tlay1 = 12)
...
heatmap5 <- ggplot(ttt_long, aes(x = variable, y = X, fill = value)) + 
  geom_tile() + 
  ...
  labs(title = "(B) Normalized Sensitivity Coefficients for laying hens (POMF)", x = "Target tissues", y = "Parameters") +
  theme_minimal() +
...
heatmap6 <- ggplot(ttt_long, aes(x = variable, y = X, fill = value)) + 
  geom_tile() + 
  scale_fill_gradient2(low = brewer.pal(11, "RdYlBu")[1], 
                       high = brewer.pal(11, "RdYlBu")[11], 
                       mid = "darkslategray3", # Shiny middle color
  ...
  labs(title = "(B) Normalized Sensitivity Coefficients for laying hens (POMW)", x = "Target tissues", y = "Parameters") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1), 
        axis.title.x = element_blank(), 
        axis.title.y = element_blank(),
...
library(gridExtra)
combined_plot <- grid.arrange(heatmap3, heatmap6, ncol = 2) 
